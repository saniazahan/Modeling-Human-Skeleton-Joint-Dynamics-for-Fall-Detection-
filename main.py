...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            for i in range(splits):
                ...
                N, C, T, V, M = batch_data.size()
                target_pred = batch_data[:,:,-10:,:,:].reshape(N*M,C,10,V)
                #print(target_pred.shape)
                # forward
                output, pred, target = self.model(batch_data, target_pred)
                ...
                if pred:
                    ...
                    prediction_loss = self.loss_pred(pred, target) / splits
                    loss  = loss + self.w_pred*prediction_loss
                ...
                if pred:
                    self.train_writer.add_scalar('Rec_loss', prediction_loss.item() * splits, self.global_step)
                    self.train_writer.add_scalar('Class_loss', classifier_loss.item() * splits, self.global_step)
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        with torch.no_grad():
            ...
            df_cm = pd.DataFrame(conf_mat, index = range(self.arg.model_args['num_class']), columns=range(self.arg.model_args['num_class']))
